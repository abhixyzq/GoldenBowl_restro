...
import { PartyPopper, Cake, Heart, Sparkles, CheckCircle, Phone, MessageSquare, ArrowRight } from 'lucide-react';
...
export const EventsPage = () => {
  ...
  return (
    <div className="pb-24 md:pb-32 space-y-xl animate-fade-in">
      ...
      {/* Bento Grid Showcase */}
      <section className="px-margin-mobile md:px-margin-desktop max-w-7xl mx-auto space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          
          {/* Main Card */}
          <div className="md:col-span-7 bg-surface-container-low p-8 rounded-3xl plate-shadow border border-outline-variant/30 flex flex-col justify-center space-y-4">
          ...
          <div className="md:col-span-5 h-64 md:h-auto rounded-3xl overflow-hidden plate-shadow border border-outline-variant/20 relative group">
            <img 
              ...
              alt="Anniversary Table Setup"
              className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700"
            />
          </div>

          {/* Quote Card */}
          <div className="md:col-span-7 bg-tertiary-container text-on-tertiary-container p-8 rounded-3xl plate-shadow flex items-center justify-between">
            <p className="font-body text-base font-medium italic leading-relaxed max-w-lg">
              "Contact us to customize your celebration. We make your special moments memorable with beautiful balloons, lights, and decor."
            </p>
            <Sparkles className="w-10 h-10 opacity-60 flex-shrink-0" />
          </div>
      ...
      {/* Packages Section */}
      <section className="py-8 px-margin-mobile md:px-margin-desktop max-w-7xl mx-auto space-y-8">
        <div className="text-center space-y-2">
          <h2 className="font-headline text-3xl font-extrabold text-on-surface">Our Decoration Packages</h2>
          <p className="text-on-surface-variant text-sm">Transparent, customizable, and visually stunning.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {DECORATION_PACKAGES.map((pkg) => (
            <div 
              key={pkg.id}
              className="bg-surface p-8 rounded-3xl plate-shadow border border-outline-variant/30 flex flex-col justify-between space-y-6 hover:-translate-y-2 transition-transform duration-300"
            >
              <div className="space-y-4">
                ...
                <div>
                  <span className="font-label text-[11px] font-bold text-secondary uppercase tracking-widest block">{pkg.tagline}</span>
                ...
                <div className="space-y-2 pt-2 border-t border-outline-variant/20">
                  {pkg.features.map((feat, idx) => (
                    <div key={idx} className="flex items-center gap-2 text-xs text-on-surface font-medium">
              ...
              <div className="pt-4 border-t border-outline-variant/20 flex items-center justify-between">
                <div>
                  <span className="text-[10px] text-on-surface-variant block font-medium">Starting from</span>
                  <span className="font-headline text-2xl font-extrabold text-primary">₹{pkg.price}</span>
                </div>

                <button
                  onClick={() => setIsEventModalOpen(true)}
                  className="bg-primary text-on-primary font-label text-xs font-bold px-5 py-2.5 rounded-full hover:bg-opacity-90 active:scale-95 transition-all shadow-sm"
                >
                  Book Setup
                </button>
              </div>
        ...
        <p className="text-center font-label text-xs text-on-surface-variant opacity-60 italic">
      ...
      <section className="px-margin-mobile md:px-margin-desktop max-w-7xl mx-auto">
        <div className="bg-inverse-surface text-inverse-on-surface rounded-3xl p-8 md:p-12 text-center space-y-4 plate-shadow">
          <h2 className="font-headline text-3xl font-extrabold">Ready to Celebrate at Golden Bowl?</h2>
          <p className="font-body text-sm text-white/90 max-w-xl mx-auto leading-relaxed">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center pt-2">
            <button
              onClick={() => setIsEventModalOpen(true)}
              className="bg-primary-container text-on-primary-container font-headline text-sm font-bold px-8 py-3.5 rounded-full flex items-center justify-center gap-2 hover:bg-primary transition-all active:scale-95 shadow-md"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="bg-surface text-on-surface font-headline text-sm font-bold px-8 py-3.5 rounded-full flex items-center justify-center gap-2 hover:bg-surface-variant transition-all active:scale-95"
            >
